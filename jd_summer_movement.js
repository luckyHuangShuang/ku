        ...
        case 'olympicgames_home':
            if (data.code === 0 && data.data && data.data.result) {
                if (data.data['bizCode'] === 0) {
                    ...
                    console.log(`团队运动互助码：${data.data.result && data.data.result.groupInfoVO.groupInviteId || '助力已满，获取助力码失败'}\n`);
                    if (data.data.result && data.data.result.groupInfoVO.groupInviteId) {
                        $.groupInviteIdList.push({
                            'ues': $.UserName,
                            // 'secretp': $.secretp,
                            'groupInviteId': data.data.result.groupInfoVO.groupInviteId,
                            'max': false
                        });
                    }
                }
        ...
        case 'help':
            if (data.data && data.data.bizCode === 0) {
                ...
                if (data.data.bizMsg.indexOf('不需要助力') > -1) {
                    $.inviteList.filter(row => row.inviteId === $.inviteId)[0].max = true
                }
                console.log(data.data.bizMsg);
            } else {
                console.log(res);
